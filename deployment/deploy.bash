...
cd ~
git clone https://github.com/SpotLabsNET/nodejs-pool.git  # Change this depending on how the deployment goes.
cd /usr/src/gtest
sudo cmake .
sudo make
...
curl https://raw.githubusercontent.com/SpotLabsNET/nodejs-pool/master/deployment/monero_daemon.patch | sudo git apply -v
sudo make -j$(nproc)
...
cd ~
git clone https://github.com/SpotLabsNET/poolui.git
cd poolui
npm install
